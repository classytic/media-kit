...
export function createMediaModels(connection: Connection, config: ResolvedMediaConfig): MediaModels {
  ...
  const schema = buildMediaSchema({
    ...
    extraFields: config.schemaOptions?.extraFields as
      | Record<string, import('mongoose').SchemaDefinitionProperty>
      | undefined,
